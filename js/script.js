...
/* ── launch-day celebration: once the countdown bar is 40% in view after
   the doors open: the digits scramble, confetti pops from the timer
   itself, and the numbers decode into a welcome message ── */
...
function armCelebration() {
  ...
  const io40 = new IntersectionObserver(es => {
    es.forEach(e => {
      if (e.isIntersecting) { io40.disconnect(); setTimeout(() => celebrate(bar), 400); }
    });
  }, { threshold: 0.4 });
  io40.observe(bar);
}

const GLYPHS = 'VELMONTIDA0123456789◆✦·';
function scrambleInto(el, finalText, dur, done) {
...
function celebrate(bar) {
  ...
  const digits = ['cd-d', 'cd-h', 'cd-m', 'cd-s'].map(id => document.getElementById(id));
  ...
  const H3 = h3.textContent, SUB = sub.textContent;

  const showWelcome = () => {
    timer.style.display = 'none';
    h3.textContent = ''; sub.textContent = '';
    welcome.style.display = 'block';
    requestAnimationFrame(() => requestAnimationFrame(() => welcome.classList.add('on')));
    scrambleInto(h3, H3, 1100, () => scrambleInto(sub, SUB, 900));
  };

  if (reduced) { showWelcome(); return; }

  /* phase 1: the zeros go haywire */
  const jumble = setInterval(() => {
    digits.forEach(d => {
      d.textContent = String((Math.random() * 10) | 0) + String((Math.random() * 10) | 0);
    });
  }, 55);

  setTimeout(() => {
    clearInterval(jumble);
    confettiBurst(timerRect);
    showWelcome();
  }, 900);
  setTimeout(() => confettiBurst(timerRect, 0.45), 1450);
}

function confettiBurst(rect, scale = 1) {
  ...
  const spawn = (x, y, dir, n) => {
    for (let i = 0; i < n; i++) {
      ...
      const sp = (4.5 + Math.random() * 6.5) * (0.8 + 0.4 * scale);
  ...
  spawn(rect.left + rect.width * 0.08, cy, -Math.PI * 0.42, n);
  spawn(rect.left + rect.width * 0.5, cy - 8, -Math.PI / 2, n);
  spawn(rect.left + rect.width * 0.92, cy, -Math.PI * 0.58, n);
  ...
  const loop = ts => {
    ...
    for (const p of parts) {
      ...
      p.vy += 0.22 * dt; p.vx *= 0.986; p.vy *= 0.992;
